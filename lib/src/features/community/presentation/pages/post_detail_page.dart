...
import 'package:cultureyo/src/features/community/presentation/pages/post_edit_page.dart'; // ⭐ [추가] 수정 페이지 임포트
...
class _PostDetailPageState extends State<PostDetailPage> {
  ...
  Future<void> _deletePostApi() async {
    try {
      final success = await _commentService.deletePost(
        widget.post.id,
        _currentUserId,
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  // ⭐ [수정 반영] 작성자에게만 수정/삭제 버튼을 보여주도록 조건부 렌더링 적용
                  if (_isAuthor)
                    Padding(
                        ...
                        children: [
                          ...
                          GestureDetector(
                            onTap: () async {
                              ...
                              // ⭐ [수정] PostEditPage로 이동하며 현재 게시물 데이터(widget.post) 전달
                              final bool? result = await Navigator.push<bool>(
                                context,
                                MaterialPageRoute(
                                  builder: (context) => PostEditPage(
                                    postToEdit: widget.post, // 수정할 Post 객체 전달
                                  ),
                                ),
                              );

                              // 수정 완료 후 true를 반환받으면 (게시물이 수정되었으면)
                              if (result == true) {
                                // 💡 중요: 실제 앱에서는 여기서 상세 정보 API를 다시 호출하여
                                //       widget.post를 새로운 데이터로 업데이트하고, setState()를 호출해야 합니다.
                                _showSnackbar('게시물이 수정되었습니다. 상세 정보 새로고침이 필요합니다.');

                                // 임시 방편으로, 실제 데이터가 변경되었으므로 사용자에게 알림만 주고
                                // 상세 정보를 다시 불러오는 로직(예: _fetchPostDetail() 같은 함수)이 추가되어야 합니다.
                              }
                            },
                            child: Container(
