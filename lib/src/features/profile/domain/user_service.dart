...
class UserService {
  ...
  Future<void> saveUserData({
    ...
    try {
      ...
      if (response.statusCode != 200) {
        // 201은 보통 '생성됨'을 의미하므로, '업데이트'에서는 200이 더 적합합니다.
  ...
  Future<Map<String, dynamic>> loadUserName() async {
    ...
    try {
      final response = await dio
          .get('/api/user/loadUserData', queryParameters: {'fields': 'name'});
      if (response.statusCode != 200 && response.statusCode != 201) {
        throw Exception('Failed to load user name: ${response.statusCode}');
      }
  ...
  Future<Map<String, dynamic>> loadUserAll() async {
    ...
    try {
      final response = await dio
          .get('/api/user/loadUserData', queryParameters: {'fields': '*'});
      if (response.statusCode != 200 && response.statusCode != 201) {
        throw Exception('Failed to load user name: ${response.statusCode}');
      }
  ...
  Future<void> deleteUser() async {
    final dio = dioClient.dio;
    try {
      final response = await dio.post('/api/user/deleteUser');
      if (response.statusCode != 200 && response.statusCode != 201) {
        throw Exception('Failed to delete user: ${response.statusCode}');
      }
      await dioClient.notifyAuthFailure();
    } on DioException catch (e) {
